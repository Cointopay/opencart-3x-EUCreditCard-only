...
class ControllerExtensionPaymentCoinToPayFiatCC extends Controller
{
    public function index()
    {
        ...
        if (isset($this->session->data['order_id'])) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . 'template/extension/payment/cointopay_fiat_cc_failed')) 
			{
				return  $this->response->setOutput($this->load->view('extension/payment/cointopay_fiat_cc_failed', $data));
			} 
			else 
			{
				return  $this->response->setOutput($this->load->view('extension/payment/cointopay_fiat_cc_failed', $data));
        ...
        try {
            ...
            if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
                if (empty($this->config->get('payment_cointopay_fiat_cc_merchantID')) || empty($this->config->get('payment_cointopay_fiat_cc_securitycode'))) {
                ...
                if ($php_arr->error == '' || empty($php_arr->error)) {
                    ...
                    $data1['PaymentDetail'] = $php_arr->PaymentDetailCConly;
                ...
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/cointopay_fiat_cc_invoice')) {
                    $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/payment/cointopay_fiat_cc_invoice', $data1));
                } else {
                    $this->response->setOutput($this->load->view('extension/payment/cointopay_fiat_cc_invoice', $data1));
                }
            } else {
                $this->load->language('extension/payment/cointopay_fiat_cc');

                $data['action'] = $this->url->link('extension/payment/cointopay_fiat_cc', '', true);
                ...
                $data['AltCoinID'] = $this->config->get('payment_cointopay_fiat_cc_crypto_coin');
                ...
                if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/cointopay_fiat_cc')) {
                    return $this->load->view($this->config->get('config_template') . '/template/extension/payment/cointopay_fiat_cc', $data);
                } else {
                    return $this->load->view('extension/payment/cointopay_fiat_cc', $data);
                }
            }
    ...
    public function c2pCurl($data, $apiKey, $post = false)
    {
        ...
        if ($post) {
            $formData = $post;
            $formData['transactionconfirmurl'] = $this->url->link('extension/payment/cointopay_fiat_cc/callback');
            $formData['transactionfailurl'] = $this->url->link('extension/payment/cointopay_fiat_cc/callback');
    ...
    public function callback()
    {
        ...
        if (isset($_REQUEST['status'])) {
            ...
            $data = [
                'mid' => $this->config->get('payment_cointopay_fiat_cc_merchantID'),
                ...
                if ($transactionData['data']['Security'] != $_REQUEST['ConfirmCode']) {
                   ...
                   $data = array();
                    ...
                    if ($_REQUEST['status'] == 'paid' && $_REQUEST['notenough'] == 0) {
                        ...
                        $this->model_checkout_order->addOrderHistory($_REQUEST['CustomerReferenceNr'], $this->config->get('payment_cointopay_fiat_cc_callback_success_order_status_id', 'Successfully Paid'),'Cointopay Transaction #'.$_REQUEST['TransactionID'].' Set to completed for OrderID #'.$_REQUEST['CustomerReferenceNr'], false);
                        $data['text_success'] = $this->language->get('text_success');
                        $data['footer'] = $this->load->controller('common/footer');
                        $data['header'] = $this->load->controller('common/header');

                        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/cointopay_fiat_cc_success')) {
                            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/payment/cointopay_fiat_cc_success', $data));
                        } else {
                            $this->response->setOutput($this->load->view('extension/payment/cointopay_fiat_cc_success', $data));
                        }
                        ...
                        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/cointopay_fiat_cc_failed')) {
                            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/extension/payment/cointopay_fiat_cc_failed', $data));
                        } else {
                            $this->response->setOutput($this->load->view('extension/payment/cointopay_fiat_cc_failed', $data));
                        }
                    } elseif ($_REQUEST['status'] == 'failed') {
                        $this->model_checkout_order->addOrderHistory($_REQUEST['CustomerReferenceNr'], $this->config->get('payment_cointopay_fiat_cc_callback_failed_order_status_id', 'Transaction payment failed'),'Cointopay Transaction #'.$_REQUEST['TransactionID'].' Set to failed for OrderID #'.$_REQUEST['CustomerReferenceNr'], false);
                        ...
                        $this->model_checkout_order->addOrderHistory($_REQUEST['CustomerReferenceNr'], 7,'Cointopay Transaction #'.$_REQUEST['TransactionID'].' Set to '.$_REQUEST['status'].' for OrderID #'.$_REQUEST['CustomerReferenceNr'], false);
                        ...
                        $this->model_checkout_order->addOrderHistory($_REQUEST['CustomerReferenceNr'], 7,'Cointopay Transaction #'.$_REQUEST['TransactionID'].' Set to canceled for OrderID #'.$_REQUEST['CustomerReferenceNr'], false);
